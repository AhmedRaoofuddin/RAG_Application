...
export async function fetchApi(endpoint: string, options: FetchOptions = {}) {
  ...
  // Build full URL - if endpoint starts with /api, prepend the backend URL
  ...
  try {
    ...
    // Backend handles guest users automatically
    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      
      // For 401 errors, just log a warning and continue (guest mode)
      if (response.status === 401) {
        console.warn('No authentication provided - continuing as guest user');
      }
      
      // Only throw errors for non-401 status codes
      if (response.status !== 401) {
        throw new ApiError(
          response.status,
