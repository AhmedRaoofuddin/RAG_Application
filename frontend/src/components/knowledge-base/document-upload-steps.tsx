...
export function DocumentUploadSteps({
  ...
  const handlePreview = async () => {
    ...
    try {
      const data = await api.post(
        `/api/knowledge-bases/${knowledgeBaseId}/documents/preview`,
        {
  ...
  const handleProcess = async (uploadResults?: UploadResult[]) => {
    ...
    try {
      const data = (await api.post(
        `/api/knowledge-bases/${knowledgeBaseId}/documents/process`,
        resultsToProcess
      )) as TaskResponse;
    ...
    const poll = async () => {
      try {
        const response = (await api.get(
          `/api/knowledge-bases/${knowledgeBaseId}/documents/tasks?task_ids=${taskIds.join(
            ","
          )}`
