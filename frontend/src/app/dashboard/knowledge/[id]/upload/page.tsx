  ...
  const handleUpload = async (file: File) => {
    ...
    try {
      const result: UploadResult = await api.post(
        `/api/knowledge-bases/${params.id}/documents/upload`,
        formData
      );
  ...
  const startProcessing = async () => {
    ...
    try {
      ...
      const response = await api.post(
        `/api/knowledge-bases/${params.id}/documents/process`,
        uploadResults
      );
  ...
  const checkProcessingStatus = async () => {
    ...
    try {
      ...
      const status: Record<string, TaskStatus> = await api.get(
        `/api/knowledge-bases/${params.id}/documents/tasks?task_ids=${taskIds}`
      );
