...
async def process_document_background(
    ...
    try:
        ...
        from pathlib import Path
        try:
            # Construct local file path from temp_path
            # temp_path is like "kb_{kb_id}/temp/{filename}"
            local_temp_path = Path("data/uploads") / temp_path
            
            # Ensure the file exists
            if not local_temp_path.exists():
                error_msg = f"File not found at {local_temp_path}"
                logger.error(f"Task {task_id}: {error_msg}")
                raise Exception(error_msg)
            
            # Convert to string for compatibility with loaders
            local_temp_path = str(local_temp_path)
            logger.info(f"Task {task_id}: Using local file: {local_temp_path}")
        except Exception as e:
            error_msg = f"Failed to access temp file: {str(e)}"
            logger.error(f"Task {task_id}: {error_msg}")
            raise Exception(error_msg)
        ...
        try:
            ...
            try:
                ...
                # 移动文件从temp到永久目录
                source_file = Path(local_temp_path)
                dest_file = permanent_dir / file_name
                
                import shutil
                shutil.move(str(source_file), str(dest_file))
                logger.info(f"Task {task_id}: File moved to permanent storage at {dest_file}")
                
                # 清理临时目录（如果为空）
                temp_dir = source_file.parent
                try:
                    if temp_dir.exists() and not any(temp_dir.iterdir()):
                        temp_dir.rmdir()
                        logger.info(f"Task {task_id}: Removed empty temp directory")
                except Exception:
                    pass  # Ignore errors when cleaning up temp dir
            ...
            try:
                if os.path.exists(local_temp_path):
                    ...
                    logger.info(f"Task {task_id}: Temp file already moved to permanent storage")
            except Exception as e:
        ...
        try:
            ...
            if temp_file.exists():
                ...
                logger.info(f"Task {task_id}: Temporary file already removed")
        except Exception as e:
            logger.warning(f"Task {task_id}: Failed to clean up temporary file after error: {str(e)}")
    finally:
