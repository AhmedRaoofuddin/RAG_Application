...
def upgrade() -> None:
    ...
    # For SQLite, these columns are already in the initial schema
    if not is_sqlite:
    ...
    # SQLite doesn't support ALTER COLUMN with SET NOT NULL
    if not is_sqlite:
    ...
    # Foreign keys need to be defined during table creation in SQLite
    if not is_sqlite:
        ...
        # For MySQL, drop the old 'metadata' column (SQLite initial schema already has 'chunk_metadata')
    ...
    # SQLite doesn't support ALTER COLUMN for changing nullability
    if not is_sqlite:
